...
// Importamos todos los iconos necesarios, incluyendo Eye y EyeOff
import { Moon, Sun, X, CheckCircle, XCircle, Eye, EyeOff } from "lucide-react"; 

// --- Componente de Notificación Simple ---
const Notification = ({ message, type, onClose }) => {
  if (!message) return null;
  // ... (cuerpo del Notification, sin cambios) ...
...
export default function Register() {
  ...
  const [confirmPassword, setConfirmPassword] = useState(""); // ⬅️ Nuevo
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-6 ${bgClass}`}>
      ...
      <div className={`w-full max-w-lg rounded-2xl p-10 animate-fadeIn ${cardClass}`}>
        ...
        <p className={`text-center text-sm mt-8 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          ¿Ya tienes cuenta?{" "}
          <a href="/" className={`font-semibold transition ${linkClass}`}>
            Inicia sesión aquí
          </a>
        </p>
      </div>
